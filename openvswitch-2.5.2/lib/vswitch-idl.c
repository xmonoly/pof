...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_AUTOATTACH]);
}
...
{
    return ovsrec_autoattach_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_AUTOATTACH]));
}
...
static void
ovsrec_bridge_parse_work_mode(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct ovsrec_bridge *row = ovsrec_bridge_cast(row_);

    ovs_assert(inited);
    if (datum->n >= 1) {
        row->work_mode = datum->keys[0].string;
    } else {
        row->work_mode = NULL;
    }
}
...
static void
ovsrec_bridge_unparse_work_mode(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_BRIDGE]);
}
...
{
    return ovsrec_bridge_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_BRIDGE]));
}
...
/* Causes the original contents of column "work_mode" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "work_mode" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction commits, then the transaction aborts and ovsdb_idl_txn_commit()
 * returns TXN_AGAIN_WAIT or TXN_AGAIN_NOW (depending on whether the database
 * change has already been received).
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "work_mode" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "work_mode" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     ovsrec_bridge_insert()).
 *
 *   - If "work_mode" has already been modified (with
 *     ovsrec_bridge_set_work_mode()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * ovsrec_bridge_set_work_mode() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
ovsrec_bridge_verify_work_mode(const struct ovsrec_bridge *row)
{
    ovs_assert(inited);
    ovsdb_idl_txn_verify(&row->header_, &ovsrec_bridge_columns[OVSREC_BRIDGE_COL_WORK_MODE]);
}
...
/* Returns the "work_mode" column's value from the "Bridge" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes work_mode's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "work_mode" member in ovsrec_bridge. */
const struct ovsdb_datum *
ovsrec_bridge_get_work_mode(const struct ovsrec_bridge *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &ovsrec_bridge_col_work_mode);
}
...
/* Sets the "work_mode" column from the "Bridge" table in 'row' to
 * the 'work_mode' set.
 *
 * If "work_mode" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 *
 * Argument constraints: either "pof" or "ovs"
 *
 * The caller retains ownership of the arguments. */
void
ovsrec_bridge_set_work_mode(const struct ovsrec_bridge *row, const char *work_mode)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (work_mode) {
        datum.n = 1;
        datum.keys = &key;
        key.string = CONST_CAST(char *, work_mode);
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &ovsrec_bridge_columns[OVSREC_BRIDGE_COL_WORK_MODE], &datum);
}
...
ovsrec_bridge_columns_init(void)
{
    ...
    /* Initialize ovsrec_bridge_col_work_mode. */
    c = &ovsrec_bridge_col_work_mode;
    c->name = "work_mode";
    ovsdb_base_type_init(&c->type.key, OVSDB_TYPE_STRING);
    c->type.key.enum_ = xmalloc(sizeof *c->type.key.enum_);
    c->type.key.enum_->n = 2;
    c->type.key.enum_->keys = xmalloc(2 * sizeof *c->type.key.enum_->keys);
    c->type.key.enum_->keys[0].string = xstrdup("ovs");
    c->type.key.enum_->keys[1].string = xstrdup("pof");
    c->type.key.enum_->values = NULL;
    ovsdb_datum_sort_assert(c->type.key.enum_, OVSDB_TYPE_STRING);
    c->type.key.u.string.minLen = 0;
    ovsdb_base_type_init(&c->type.value, OVSDB_TYPE_VOID);
    ...
    c->type.n_max = 1;
    c->mutable = true;
    c->parse = ovsrec_bridge_parse_work_mode;
    c->unparse = ovsrec_bridge_unparse_work_mode;
}

/* Controller table. */
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_CONTROLLER]);
}
...
{
    return ovsrec_controller_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_CONTROLLER]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_FLOW_SAMPLE_COLLECTOR_SET]);
}
...
{
    return ovsrec_flow_sample_collector_set_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_FLOW_SAMPLE_COLLECTOR_SET]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_FLOW_TABLE]);
}
...
{
    return ovsrec_flow_table_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_FLOW_TABLE]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_IPFIX]);
}
...
{
    return ovsrec_ipfix_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_IPFIX]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_INTERFACE]);
}
...
{
    return ovsrec_interface_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_INTERFACE]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_MANAGER]);
}
...
{
    return ovsrec_manager_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_MANAGER]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_MIRROR]);
}
...
{
    return ovsrec_mirror_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_MIRROR]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_NETFLOW]);
}
...
{
    return ovsrec_netflow_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_NETFLOW]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_OPEN_VSWITCH]);
}
...
{
    return ovsrec_open_vswitch_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_OPEN_VSWITCH]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_PORT]);
}
...
{
    return ovsrec_port_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_PORT]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_QOS]);
}
...
{
    return ovsrec_qos_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_QOS]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_QUEUE]);
}
...
{
    return ovsrec_queue_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_QUEUE]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_SSL]);
}
...
{
    return ovsrec_ssl_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_SSL]));
}
...
{
    return ovsdb_idl_table_get_seqno(idl, &ovsrec_table_classes[OVSREC_TABLE_SFLOW]);
}
...
{
    return ovsrec_sflow_cast(ovsdb_idl_track_get_first(idl, &ovsrec_table_classes[OVSREC_TABLE_SFLOW]));
}
